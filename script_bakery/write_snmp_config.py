...
if __name__ == "__main__":
    ...
    if args.loglevel is None:
        loglevel = tools.get_loglevel(tools.get_value_from_dict(local_snmp_config_file, ['general', 'logging', 'level']))
    else:
        loglevel = tools.get_loglevel(args.loglevel)

    log_format = tools.get_value_from_dict(local_snmp_config_file, ['general', 'logging', 'format'])
    if log_format is None:
        log_format = '%(asctime)s %(levelname)s:%(message)s'
    logfile = tools.get_value_from_dict(local_snmp_config_file, ['general', 'logging', 'filename'])
    ...
    if args.profile is not None:
        username = local_snmp_config_file.get('profiles',{}).get(args.profile,{}).get('username')
        token = local_snmp_config_file.get('profiles',{}).get(args.profile,{}).get('password')
